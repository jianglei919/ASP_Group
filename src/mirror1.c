...
static int send_heartbeat_once(void)
{
    ...
    /*
     * 发送 "HEARTBEAT mirror1\n"，w26server 的 crequest() 会识别 HEARTBEAT 前缀，
     * 更新状态文件中 mirror1 的时间戳并回复 HB_OK，不消耗客户端连接序号。
     */
...
/*
 * 功能：处理单条客户端命令并返回响应。
 * 实现原理：mirror1 作为最终设计中的真实业务节点，直接在本地完成控制命令、目录检索和归档命令处理；
...
/*
 * 功能：程序入口，初始化配置并启动 mirror1 服务端。
 * 实现原理：mirror1 先启动心跳线程向主服上报在线状态，再进入监听循环，作为最终设计中的实际业务节点
 * 直接处理其归属连接。
 */
int main(void)
{
