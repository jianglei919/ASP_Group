...
int main(void)
{
    ...
    session_fd = connect_to_server(PRIMARY_HOST, g_primary_port);
    if (session_fd < 0)
    {
        fprintf(stderr, "client: failed to connect w26server (%s:%d)\n", PRIMARY_HOST, g_primary_port);
        return 1;
    }
     ...
     */
    {
        ...
        printf("client connected to w26server (%s:%d), NODE: %s\n",
               current_host, current_port,
               (current_port == g_primary_port)   ? "w26server"
               : (current_port == g_mirror1_port) ? "mirror1"
                                                  : "mirror2");
        fflush(stdout);
    }
